...
week = scheduledata["weeks"][(new Date().getFullYear()).toString()][(new Date().getWeekNumber()).toString()]
...
day.forEach(function(scheduleelement){
  ...
  scheduleelementdiv.style.top=scheduleelement["start"]+"px"
  console.log(scheduleelement["start"]-scheduleelement["end"])
...
document.getElementById("FileAttachment").addEventListener('change', onChange);
});
