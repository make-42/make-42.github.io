...
function displayresults(){var typingresults=document.getElementsByClassName("typing-results")[0]
...
function updateunderscoreposition(){var typingwords=document.getElementsByClassName("typing-words")[0]
...
function updateunderscorescale(){var typingletterindicator=document.getElementsByClassName("typing-letter-indicator")[0];typingletterindicator.style.transform="scaleX("+(teststatus+Math.abs(scaleoffset))+")";scaleoffset/=1.5;}
function setuptypingtest(){testtarget="";testposition=0;testerrors=0;teststatus=1;var typingwords=document.getElementsByClassName("typing-words")[0]
typingwords.innerHTML="";var i;for(i=0;i<words;i++){word=wordlist.sample()
var j;for(j=0;j<word.length;j++){typingwords.innerHTML+="<span class=\"typing-letter-unconfirmed\">"+word[j]+"</span>"}
if(i<words-1){testtarget+=word+" ";typingwords.innerHTML+="<span class=\"typing-letter-unconfirmed\">&nbsp;</span>"}else{testtarget+=word;}}
typingwords.innerHTML+="<span class=\"typing-letter-indicator\">&nbsp;</span>";testlength=testtarget.length;}
function displayhash(){alert(sha256(timecompleted+";"+testtarget+";"+testposition+";"+lastposition+";"+testlength+";"+teststartdate+";"+testerrors))
console.log(timecompleted+";"+testtarget+";"+testposition+";"+lastposition+";"+testlength+";"+teststartdate+";"+testerrors)}
$(document).ready(function(){setuptypingtest();setInterval(updateunderscoreposition,50);setInterval(updateunderscorescale,50);});document.addEventListener('keydown',function(event){if(testposition+1==testlength){teststatus=0;}
if(testposition==testlength){teststatus=0;console.log("test end")
if(event.keyCode==27){setuptypingtest();}
else if(event.keyCode==118){displayhash();}}else{var typingwords=document.getElementsByClassName("typing-words")[0]
if(testposition==0){teststartdate=Date.now();}
if(String.fromCharCode(event.keyCode).toLowerCase()==testtarget[testposition]){typingwords.children[testposition].className="typing-letter-confirmed";testposition++;scaleoffset++;}else if(event.keyCode==8){if(testposition!=0){if(typingwords.children[testposition-1].className=="typing-letter-error"){testerrors--;scaleoffset--;}
typingwords.children[testposition-1].className="typing-letter-unconfirmed";testposition--;scaleoffset--;}}
else{typingwords.children[testposition].className="typing-letter-error";testposition++;testerrors++;scaleoffset++;}
displayresults()
